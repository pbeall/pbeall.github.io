...
\end{enumerate}
This is known as ``transfinite induction.'' Actually, the above three steps can be condensed into one step, and this one step and be split into three cases, depending on whether $\beta$ is $0$, a successor ordinal, or a limit ordinal.
...
\end{fact}
\begin{fact}
	Each ordinal $\alpha$ has an immediate successor $S(\alpha)$, i.e. $\alpha<\beta$ implies $\alpha <S(\alpha)\leq\beta$.
\end{fact}
\begin{fact}
...
\begin{axiom}[Choice]
	Suppose $\mathcal{F}$ is a nonempty family of nonempty sets, i.e. $\mathcal{F}$ is a nonempty set and $X$ is a nonempty set for every $X\in\mathcal{F}$. Then $\prod_{X\in\mathcal{F}} X$ is nonempty, i.e. there exists a map $f: \mathcal{F}\rightarrow\bigcup\mathcal{F}$ such that $f(X)\in X$ for every $X\in\mathcal{F}$.
\end{axiom}
...
\]
be a map such that $f(X)\in X$ for every $X\in\mathcal{P}(V)\setminus\{\emptyset\}$. Note that the definition of vector space implies that $V$ is nonempty, whence $\mathcal{P}(V)\setminus\{\emptyset\}$ is nonempty.

Define $I_\alpha$ for all ordinals $\alpha$ by transfinite recursion as follows. Let $I_0 = \emptyset$. If $I_\alpha$ has been defined, then let
\[
...
\]
whence $T$ is linearly independent by Result 1 and $I_\beta$ is linearly independent by Result 1. Otherwise, $\beta = S(\alpha)$ for some $\alpha$. If $I_\beta = I_\alpha$, then $I_\beta$ is immediately linearly independent. Otherwise, $I_\beta = I_\alpha\cup\{f(V\setminus\spn I_\alpha)\}$ is linearly independent by Result 2. It follows that $I_\alpha$ is linearly independent for all ordinals $\alpha$.
...
\]
and thus $I_\alpha = I_{S(\alpha)}$. If $\spn I_\alpha\subsetneq V$, then $f(V\setminus\spn I_\alpha)\in I_{S(\alpha)}\setminus I_\alpha$, contradicting $I_\alpha = I_{S(\alpha)}$. So, we must have $\spn I_\alpha = V$. Hence $I_\alpha$ is a basis for $V$.
\end{proof}

\begin{remark}
	If $V$ happens to already be well-ordered, then $f$ can be replaced by $\min$ and we can construct a basis for $V$ without using Choice.
\end{remark}
